...
/****
DCM - Summary Compliance by CI for a Configuration Baseline 
****/
          ...
          select 
			  ...
			  ,CollecionName=coll.Name
			  ,CI_ID=ci.CI_ID
			  ,ci.CIVersion
			  ,CI_UniqueID
			  , Level=0
			  , convert(varchar(1024),(ci.CI_ID)), dbo.fn_GetLocalizedCIName(1033,ci.CI_ID)
          from
          ...
          inner join  v_ConfigurationItems  ci on ci.CI_ID=rel.ReferencedCI_ID          
          where CIType_ID=2
          --and (dbo.fn_GetLocalizedCIName(1033,ci.CI_ID) = @name  or @name = '')
		...
		--  and ci.CI_ID = @BaselineID
          ...
          select
			  ...
			  , deepsortorder+'-'+convert(varchar(32),rci.ToCIID)), dbo.fn_GetLocalizedCIName(1033,ci.CI_ID)
          from AllBaselines bci
          inner join v_CIRelation  rci on rci.FromCIID=bci.CI_ID and rci.RelationType=1
          inner join v_ConfigurationItems  ci on ci.CI_ID=rci.ToCIID
          ...
          Insert into @AllCIs
          select distinct rel.FromCIID,rel.ToCIID,ciref.ModelId,ciref.CIVersion,ciref.CI_UniqueID,ciref.CIType_ID,rel.RelationType,dbo.fn_GetLocalizedCIName(1033,bl.CI_ID) as BLDisplayName, dbo.fn_GetLocalizedCIName(1033,ciref.CI_ID), dbo.fn_BuildLocalizedCICategories(ciref.CI_ID, 1033)
           from
          @allassignments bl
          inner join  v_CIRelation  rel on rel.FromCIID=bl.CI_ID
          inner join v_ConfigurationItems  ciref on ciref.CI_ID=rel.ToCIID
          left join
          (
          select distinct CI_ID from fn_CICategoryInfo(1033)  cat
          ...
          (
              ...
              join v_Collection coll on coll.CollectionID = cm.CollectionID
          ...
          (   
              ...
              join v_Collection coll on coll.CollectionID = resources.CollectionID
              where AllCIs.CI_ModelID=@temp_ModelID and AllCIs.CIVersion=@temp_CIVersion
          ) users             
          left join
